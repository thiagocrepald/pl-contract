...
const DoctorsList = () => {
    ...
    const handleTransformToTableContent = (content?: Contract[]): (string | JSX.Element)[][] => {
        ...
        return content.map((item, index) => [
            ...
            item.status === ContractState.ACTIVE ? (
                <div className="active-tag">{t('contracts.tableContent.status.active')}</div>
            ) : (
                <div className="inactive-tag">{t('contracts.tableContent.status.inactive')}</div>
            ),
            <Fragment key={index}>
        ...
        <div className="doctor__container">
            ...
            <div className={'doctor-list'} style={{ padding: '0 26px', overflow: 'auto' }}>
                ...
                <Menu className="tooltip-style" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleClose}>
                    ...
                    <MenuItem onClick={onActivateContract}>
                        {t(`management.buttons.${selectedContract?.status === ContractState.ACTIVE ? 'deactivate' : 'activate'}`)}
                    </MenuItem>
                </Menu>
            </div>
        </div>
